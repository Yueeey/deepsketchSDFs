...
def main_function(experiment_directory, continue_from):
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, image, sil, azi, name in sdf_loader:
            ...
            reg_list = []
            sigma = 0.11 * 0.997/3
            vecs_nm = F.normalize(vecs)
            klloss = nn.KLDivLoss()
            sdf_reg = sdf_data[:, :, 3].cuda()

            for n in range(batch_size): 
                t_feature_list = torch.stack([vecs_nm[n]]*(batch_size-1), dim=0)
                s_feature_list = torch.cat([vecs_nm[0:n], vecs_nm[n+1:]], dim=0)
                t_points_list = torch.stack([sdf_reg[n]]*(batch_size-1), dim=0)
                s_points_list = torch.cat([sdf_reg[0:n], sdf_reg[n+1:]], dim=0)
                p_deno = torch.sum(torch.exp(-1 * loss_l1(t_points_list, s_points_list) / (2 * sigma)))
                p_hat_deno = torch.sum(torch.einsum('bl, bl -> b' , t_feature_list, s_feature_list))
                p = torch.exp(-1 * loss_l1(t_points_list, s_points_list) / (2 * sigma))/ p_deno
                p_hat = torch.einsum('bl, bl -> b' , t_feature_list, s_feature_list)/ p_hat_deno
                # reg_t = torch.mean(torch.abs(p_hat-p))
                reg_t = klloss(p, p_hat)
